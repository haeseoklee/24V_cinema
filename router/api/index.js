...
router.get("/selected_movie/:id", function (req, res) {
    ...
    var sql =
        "select distinct s.id, s.name, s.cinema " +
...
router.get("/selected_movie/:movie_id/selected_screen/:screen_id", function (
    req,
    res
) {
    ...
    var sql =
        "select t.id, t.startdate, t.starttime, t.enddate, t.endtime, m.title, s.name " +
        "from screen s join timetable t on t.screen_id = s.id " +
        ...
        if (err) throw err;
            ...
            var query = db.query(sql, function (err, rows) {
                if (err) throw err;
                    ...
                    var sql =
                        "SELECT coupon.id, coupon.name, coupon.benefit FROM coupon " +
                    ...
                    var query = db.query(sql, [req.user], function (err, rows) {
                        if (err) throw err;
                            ...
                            resData.coupon_list = JSON.parse(
                                JSON.stringify(rows)
                            );
                            res.json(resData);
                        }
        ...
        if (err) throw err;
            ...
            if (pw.substring(0, 2) === req.body.password_two) {
                if (req.body.couponId !== "0") {
                    sql =
                        "DELETE FROM customer_coupon WHERE customer_id=? AND coupon_id=?";
                    var query = db.query(
                        sql,
                        [req.user, req.body.couponId],
                        function (err, rows) {
                            if (err) throw err;
                        }
                    );
                        ...
                        var query = db.query(sql, data, function (err, rows) {
                            if (err) throw err;
                                ...
                                var temp = {
                                    point:
                                        point +
                                        (Number(req.body.origin_pay) -
                                            Number(req.body.disc_pay)) *
                                            0.05,
                                };
                                var query = db.query(sql, temp, function (
                                    err,
                                    rows
                                ) {
                                    if (err) throw err;
